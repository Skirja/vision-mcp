import { z } from "zod";
export function registerCodeGenerationPrompt(server) {
    server.registerPrompt("code-generation-prompt", {
    ...
    }, ({ framework, component_type, style_preference }) => ({
        messages: [
            {
                role: "user",
                content: {
                    type: "text",
                    text: `Given the visual analysis results, generate ${framework} code${component_type ? ` for component type ${component_type}` : ""}. ${style_preference ? `Style preference: ${style_preference}.` : ""}`
                }
            }
        ]
    }));
